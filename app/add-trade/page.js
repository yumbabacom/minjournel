...
export default function AddTrade() {
  ...
  const selectTradingPair = (pair) => {
    const updatedData = { ...formData, tradingPair: pair };
    setFormData(updatedData);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const tradeData = {
        ...
        accountId: formData.accountId, // This is the key field for account association
        ...
        images: formData.screenshot ? [formData.screenshot] : []
      };
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              ...
              <div className="p-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="space-y-3">
                    <label className="block text-sm font-bold text-gray-700">Account</label>
                    <div className="relative account-dropdown">
                      ...
                      >
                        {formData.accountId && accounts.find(acc => acc.id === formData.accountId) ? (
                          <div className="flex items-center space-x-3">
                            <div className={`w-10 h-10 ${accounts.find(acc => acc.id === formData.accountId)?.color || 'bg-blue-600'} rounded-lg flex items-center justify-center`}>
                            ...
                            <div className="flex-1">
                              <div className="flex items-center space-x-2">
                                <p className="font-semibold text-gray-900">{accounts.find(acc => acc.id === formData.accountId)?.name}</p>
                                {accounts.find(acc => acc.id === formData.accountId)?.tag && (
                                  <span className={`px-2 py-0.5 rounded text-xs font-medium border ${getTagColorClasses(accounts.find(acc => acc.id === formData.accountId)?.tag)}`}>
                                    {getTagDisplayName(accounts.find(acc => acc.id === formData.accountId)?.tag)}
                                  </span>
                                )}
                              </div>
                              <p className="text-sm text-gray-500">${Number(accounts.find(acc => acc.id === formData.accountId)?.balance || 0).toLocaleString()}</p>
                            </div>
                            <svg className={`w-5 h-5 text-gray-400 transition-transform duration-200 ${showAccountDropdown ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          ...
                          <div className="flex items-center space-x-3">
                            ...
                            <div className="flex-1">
                              <p className="font-semibold text-gray-500">Choose Account</p>
                              <p className="text-sm text-gray-400">Select your trading account</p>
                            </div>
                            <svg className={`w-5 h-5 text-gray-400 transition-transform duration-200 ${showAccountDropdown ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                            </svg>
...
• Key takeaways for future trades"
            ...
            <div className="px-6 py-6 space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {['personal', 'funded', 'demo', 'forex', 'crypto'].map((tag) => (
                    <button
                      key={tag}
                      type="button"
                      onClick={() => setNewAccount({ ...newAccount, tag })}
      ...
      {/* Account Dropdown - Fixed Position to Avoid Z-Index Issues */}
      {showAccountDropdown && (
        <div className="fixed inset-0 z-[9999]" onClick={() => setShowAccountDropdown(false)}>
          <div
                ...
                accounts.map((account) => (
                  <button
                    ...
                    className={`w-full p-4 text-left hover:bg-gray-50 transition-colors border-b border-gray-100 last:border-b-0 ${
                      formData.accountId === account.id ? 'bg-blue-50' : ''
                    }`}
                  >
                    <div className="flex items-center space-x-3">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center space-x-2">
                          <p className="text-sm text-gray-600">${Number(account.balance || 0).toLocaleString()}</p>
                          {account.tag && (
                            <span className={`px-2 py-0.5 rounded text-xs font-medium border ${getTagColorClasses(account.tag)}`}>
